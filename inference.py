...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for p in self.legalPositions:
      # Update the probability distribution of the ghosts' positions based on the observation
  ...
  def elapseTime(self, gameState):
    ...
    possiblePlaces = util.Counter()
    pacmanPosition = gameState.getPacmanPosition()

    #newPosDist[p] = P(G(t+1)=p | G(t) = oldPos)
    for oldPos in self.legalPositions:
        alteredState = self.setGhostPosition(gameState, oldPos)
        newPosDist = self.getPositionDistribution(alteredState) # from the old position, where could the ghost go?
        for newPos in newPosDist:
            possiblePlaces[newPos] += newPosDist[newPos]* self.beliefs[oldPos]
    possiblePlaces.normalize()
    self.beliefs = possiblePlaces
...
class JointParticleFilter:
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
        ...
        # Go through all the ghosts and update in the list of ghost positions
        for i in range(self.numGhosts):
  ...
  def observeState(self, gameState):
    ...
    for ghost in range(self.numGhosts):
        ...
        if noisyDistances[ghost] == None:
            for position in beliefDistribution:
                ghostPosition = position[ghost]
                beliefDistribution[position] *= emissionModels[position][util.manhattanDistance(ghostPosition,pacmanPosition)]
    for ghost in range(self.numGhosts):
        if noisyDistances[ghost] == None:
            newDistribution = util.Counter()
            for position in beliefDistribution:
                position=list(position)
                position[ghost] = ((2*ghost -1),1)
                newDistribution[position] += beliefDistribution[position]

    if beliefDistribution.totalCount() == 0:
        self.initializeParticles()
    else:
        for p in xrange(self.numParticles):
            self.particles[p] = util.sample(beliefDistribution)
